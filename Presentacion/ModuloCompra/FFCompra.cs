...
{
    public partial class FFCompra : Form
    {
        ...
        private static FFCompra _Instancia;

        public static FFCompra GetForm()
        {
            if (_Instancia == null)
            {
                _Instancia = new FFCompra();
            }
            return _Instancia;
        }
        ...
        internal void Limpiar()
        {
            ...
            TxtCod_fcompra.Text = string.Empty;
            TxtDesc_fcompra.Text = string.Empty;
        ...
        private void Listar()
        {
            try
            {
                DataSet DsListar = EFCompra.Listar();

                TxtCod_fcompra.Text = DsListar.Tables[0].Rows[0]["Cod_fcompra"].ToString();
                ...
                TxtDesc_fcompra.Text = DsListar.Tables[0].Rows[0]["Desc_fcompra"].ToString();
                ...
                TxtCod_fcomprav.Text = DsListar.Tables[0].Rows[0]["Cod_fcompra"].ToString();
        ...
        private void GenerarCod()
        {
            ...
            try
            {
                int rpta = Convert.ToInt32(EFCompra.GenerarCod(TxtCod_fcompra.Text));
                TxtCod_fcompra.Text = Pref + rpta.ToString(TxtCadena.Text);
            }
            catch (Exception Ex)
            {
                MensajeError(" Solo se puede Generar Codigos Numericos " + Ex.StackTrace);
                TxtCod_fcompra.Focus();
            }
        }
        ...
        {
            TxtCod_fcompra.Enabled = valor;
            TxtDesc_fcompra.Enabled = valor;
        ...
        internal void Botones()
        {
            ...
            else if (TxtEditar.Text == "Listar" && TxtEstatus.Text == "Procesado Parcial" && Importar == false)
            {
                ...
                ImportarChk.Enabled = false;
                BtnAceptarImp.Visible = false;
                Habilitar(false);
            }
        }

        private void FFCompra_Load(object sender, EventArgs e)
        {
        ...
        private void FFCompra_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;
        }

        private void FFCompra_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        #region BOTONES
        internal void MetodoProcesarDetalle()
        {
            string Rpta = "";
            DataTable DtPro = EFCompraDetalle.Mostrar(TxtCod_fcompra.Text);
            try
            {
                foreach (DataRow DGVRow in DtPro.Rows)
                {
                    Rpta = EFCompraDetalle.ProcesarDetalle(
                        Convert.ToInt32(DGVRow["reng"]),
                        Convert.ToString(DGVRow["cod_deposito"]),
                        Convert.ToString(DGVRow["cod_articulo"]),
                        Convert.ToDecimal(DGVRow["equivalencia"]));
                }
            }
            catch (Exception)
            {
        ...
        internal void Guardar()
        {
            try
            {
                ...
                if (TxtCod_proveedor.Text == string.Empty || TxtNumdocumento.Text == string.Empty)
                {
                    ...
                    ErrorTexto.SetError(TxtCod_fcompra, "Campo Obligatorio");
                ...
                else if (registrar)
                {
                    ...
                    {
                        GenerarCod();
                        Rpta = EFCompra.Insertar
                            (TxtCod_fcompra.Text, TxtCod_proveedor.Text, TxtCod_condicionpago.Text, TxtDesc_fcompra.Text, TxtOptipo.Text, TxtNumdocumento.Text, TxtNumcontrol.Text, TxtDoc_asociado.Text, TxtOrigen.Text, TxtEstatus.Text, Convert.ToBoolean(TxtAnulado.Text), Convert.ToDecimal(TxtMonto_base.Text), Convert.ToDecimal(TxtMonto_r.Text), Convert.ToDecimal(TxtMonto_d.Text), Convert.ToDecimal(TxtMonto_o.Text), Convert.ToDecimal(TxtMonto_imp.Text), Convert.ToDecimal(TxtSubtotal.Text), Convert.ToDecimal(TxtTotal.Text), Convert.ToDateTime(TxtFecha_doc.Text), Convert.ToDateTime(TxtFecha_reg.Text), Convert.ToDateTime(TxtFecha_ven.Text), TxtCampo1.Text, TxtCampo2.Text, TxtCampo3.Text, TxtCampo4.Text, TxtCampo5.Text, TxtCampo6.Text, TxtCod_usuario.Text);
                        GuardarDetalle();
                        TxtCod_fcomprav.Text = TxtCod_fcompra.Text;
                        MostrarDetalle();
                        MostrarListado();
                    } 
                    else if (TxtEditar.Text == "Editar")
                    {
                        Rpta = EFCompra.Editar
                            (TxtCod_fcompra.Text, TxtCod_fcomprav.Text, TxtCod_proveedor.Text, TxtCod_condicionpago.Text, TxtDesc_fcompra.Text, TxtOptipo.Text, TxtNumdocumento.Text, TxtNumcontrol.Text, TxtDoc_asociado.Text, TxtOrigen.Text, TxtEstatus.Text, Convert.ToBoolean(TxtAnulado.Text), Convert.ToDecimal(TxtMonto_base.Text), Convert.ToDecimal(TxtMonto_r.Text), Convert.ToDecimal(TxtMonto_d.Text), Convert.ToDecimal(TxtMonto_o.Text), Convert.ToDecimal(TxtMonto_imp.Text), Convert.ToDecimal(TxtSubtotal.Text), Convert.ToDecimal(TxtTotal.Text), Convert.ToDateTime(TxtFecha_doc.Text), Convert.ToDateTime(TxtFecha_reg.Text), Convert.ToDateTime(TxtFecha_ven.Text), TxtCampo1.Text, TxtCampo2.Text, TxtCampo3.Text, TxtCampo4.Text, TxtCampo5.Text, TxtCampo6.Text, TxtCod_usuario.Text);
                        TxtCod_fcomprav.Text = TxtCod_fcompra.Text;
                        MostrarDetalle();
                        MostrarListado();
                    }
                    ...
                    else if (TxtDoc_asociado.Text == "PCompra")
                    {
                        DialogResult Opcion = MessageBox.Show("Desea Procesar la Factura de Compra ?, Atencion este Proceso es irreversible y no puede Anularse ni Eliminarse Una vez Procesardo", "...::: Step One Ver 1.0 :::...", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);

                        if (Opcion == DialogResult.OK)
                        {
                            MetodoProcesarDetalle();
                            Rpta = EFCompra.Procesar(TxtCod_fcompra.Text, "Procesado Parcial", TxtCod_usuario.Text);
                            foreach (DataGridViewRow Row in DataListado.Rows)
                            {
                                Rpta = EPCompraDetalle.Importar(
                                    Convert.ToInt32(Row.Cells["rorigen"].Value),
                                    Convert.ToString(Row.Cells["cod_deposito"].Value),
                                    Convert.ToString(Row.Cells["cod_articulo"].Value),
                                    Convert.ToDecimal(Row.Cells["equivalencia"].Value));

                                DataTable DtD = EPCompraDetalle.Mostrar(TxtOrigen.Text);
                                ...
                                if (Validar2 == Validar1)
                                {
                                    Rpta = EPCompra.Procesar(TxtOrigen.Text, "Procesado", TxtCod_usuario.Text);
                                }
                                else if (Validar1 < Validar2)
                                {
                                    Rpta = EPCompra.Procesar(TxtOrigen.Text, "Procesado Parcial", TxtCod_usuario.Text);
                                }

                            }
                        }
                    }
                    else if (TxtDoc_asociado.Text == string.Empty)
                    {
                        MetodoProcesarDetalle();
                        Rpta = EFCompra.Procesar(TxtCod_fcompra.Text, "Procesado Parcial", TxtCod_usuario.Text);
                        TxtEstatus.Text = "Procesado Parcial";
                        MostrarListado();
                    }
        ...
        private void BtnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    if (TxtDoc_asociado.Text == "PCompra")
                    {
                        foreach (DataGridViewRow Row in DataListado.Rows)
                        {
                            Rpta = EPCompraDetalle.EliminarImportar(
                            Convert.ToInt32(Row.Cells["rorigen"].Value),
                            Convert.ToString(Row.Cells["cod_deposito"].Value),
                            Convert.ToString(Row.Cells["cod_articulo"].Value),
                            Convert.ToDecimal(Row.Cells["equivalencia"].Value));
                            ...
                            MensajeOk(Validar1.ToString());

                            if (Validar2 == Validar1)
                            {
                                Rpta = EPCompra.Procesar(TxtOrigen.Text, "Sin Procesar", TxtCod_usuario.Text);
                            }
                            else if (Validar1 < Validar2)
                            {
                                Rpta = EPCompra.Procesar(TxtOrigen.Text, "Procesado Parcial", TxtCod_usuario.Text);
                    ...
                    EliminarRenglon();
                    Rpta = EFCompra.Eliminar(TxtCod_fcompra.Text);
            ...
            try
            {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow Row in DataListado.Rows)
                    {
                        ...
                        {
                            Rpta = EFCompraDetalle.Anular(Convert.ToString(Row.Cells["cod_fcompra"].Value));
                        }
                    }

                    Rpta = EFCompra.Anular(TxtCod_fcompra.Text, TxtEstatus.Text, Convert.ToBoolean(TxtAnulado.Text), TxtCod_usuario.Text);
        ...
        private void TxtDesc_fcompra_TextChanged(object sender, EventArgs e)
        {
            CambiosTexBox();
        }

        private void TxtCod_fcomprav_TextChanged(object sender, EventArgs e)
        {
            CambiosTexBox();
        }
        ...
        private void TxtCod_fcompra_TextChanged(object sender, EventArgs e)
        {
            CambiosTexBox();
        }
        ...
        private void BtnPrimero_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet DsListar = EFCompra.Primero();
        ...
        private void BtnAnterior_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet DsListar = EFCompra.Anterior(TxtCod_fcompra.Text);
        ...
        private void BtnSiguiente_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet DsListar = EFCompra.Siguiente(TxtCod_fcompra.Text);
        ...
        private void BtnUltimo_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet DsListar = EFCompra.Ultimo();
        ...
        private void OcultarColuumnasGen()
        {
            ...
            DataListadoGen.Columns["cod_fcompra"].Visible = true;
            DataListadoGen.Columns["desc_fcompra"].Visible = false;
            ...
            DataListadoGen.Columns["cod_fcompra"].HeaderText = "Codigo";
        ...
        {
            DataListadoGen.DataSource = EFCompra.Mostrar();
            OcultarColuumnasGen();
        ...
        private void SetFCompra()
        {
            ...
            TxtCod_fcompra.Text = Convert.ToString(DataListadoGen.CurrentRow.Cells["cod_fcompra"].Value);
            TxtDesc_fcompra.Text = Convert.ToString(DataListadoGen.CurrentRow.Cells["desc_fcompra"].Value);
        ...
        private void Buscar()
        {
            ...
            {
                DataListadoGen.DataSource = EFCompra.Buscar(TxtBuscar.Text, "cod");
            }
            else if (CbBucar.Text == "Proveedor")
            {
                DataListadoGen.DataSource = EFCompra.Buscar(TxtBuscar.Text, "Desc");
            }
            else if (CbBucar.Text == "Rif")
            {
                DataListadoGen.DataSource = EFCompra.Buscar(TxtBuscar.Text, "rif");
            }
            else if (CbBucar.Text == "Nro. Cotizacion")
            {
                DataListadoGen.DataSource = EFCompra.Buscar(TxtBuscar.Text, "num");
            }
            OcultarColuumnasGen();
        }
        ...
        {
            DataListado.DataSource = EFCompraDetalle.Mostrar(TxtCod_fcompra.Text);
            OcultarColumnas();
        }
        ...
        internal void OcultarColumnas()
        {
            ...
            DataListado.Columns["cod_fcompra"].Visible = false;
        ...
        {
            DtDetalle = new DataTable("DtFCompraDetalle");

            DtDetalle.Columns.Add("reng", Type.GetType("System.Int32"));
            DtDetalle.Columns.Add("cod_fcompra", Type.GetType("System.String"));
        ...
        internal void EliminarRenglon()
        {
            foreach (DataGridViewRow Row in DataListado.Rows)
            {
                string Rpta = EFCompraDetalle.Eliminar(Convert.ToInt32(Row.Cells["reng"].Value));
            }
        }
        ...
        public void GuardarDetalle()
        {
            try
            {
                string Rpta = "";
                Int32 _rorigen;
                string _cod_fcompra, _cod_deposito, _cod_articulo, _cod_unidad, _cod_impuesto, _optipo, _estatus;
                decimal _cantidad, _equivalencia, _montound, _descuento, _descuentop, _subtotalund, _montoimp, _totalund;

                foreach (DataGridViewRow row in DataListado.Rows)
                {
                    _cod_fcompra = TxtCod_fcompra.Text;
                    ...
                    Rpta = EFCompraDetalle.Insertar(_cod_fcompra, _cod_deposito, _cod_articulo, _cod_unidad, _cod_impuesto, _optipo, _cantidad, _equivalencia, _montound, _descuento, _descuentop, _subtotalund, _montoimp, _totalund, _estatus, _rorigen);
                }
            }
            catch (Exception Exc)
            {
                    ...
                    else if (DataListado.Rows.Count > 1)
                    {
                        ...
                        if (Opcion == DialogResult.OK)
                        {
                            ...
                            foreach (DataGridViewRow Row in DataListado.Rows)
                            {
                                ...
                                {
                                    Rpta = EFCompraDetalle.Eliminar(Convert.ToInt32(Row.Cells["reng"].Value));
                                }
                            }
        ...
        private void DataListado_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string reng, cod_fcompra, cod_deposito, cod_impuesto, optipo, cod_articulo, desc_articulo, desc_deposito, cod_unidad, desc_unidad, cantidad, equivalencia, montound, descuento, descuentop, subtotalund, desc_impuesto, tasaimp, montoimp, totalund;

                if (TxtEditar.Text == "Nuevo" && TxtEstatus.Text == "Sin Procesar" && Importar == false)
                {
                    FArtCargarC mForm = FArtCargarC.GetArtCargarC();
                    mForm.TxtFlag.Text = "FCompra";
                    mForm.TxtEditar.Text = "Nuevo";
                    reng = Convert.ToString(DataListado.CurrentRow.Cells["reng"].Value);
                    cod_fcompra = Convert.ToString(DataListado.CurrentRow.Cells["cod_fcompra"].Value);
                    ...
                    QuitarRenglon();
                    mForm.SetColumnas(reng, cod_fcompra, cod_deposito, cod_impuesto, optipo, cod_articulo, desc_articulo, desc_deposito, cod_unidad, desc_unidad, cantidad, equivalencia, montound, descuento, descuentop, subtotalund, desc_impuesto, tasaimp, montoimp, totalund);
                    mForm.ShowDialog();
                }
                else if (TxtEditar.Text == "Listar" && TxtEstatus.Text == "Sin Procesar" && Importar == false)
                {
                    ...
                    mForm.TxtCod_compra.Text = TxtCod_fcompra.Text;
                    ...
                    string Rpta = EFCompraDetalle.Eliminar(Convert.ToInt32(DataListado.CurrentRow.Cells["reng"].Value));
                    MostrarDetalle();
                    CalcularDetalle();
                    mForm.SetColumnas(reng, cod_fcompra, cod_deposito, cod_impuesto, optipo, cod_articulo, desc_articulo, desc_deposito, cod_unidad, desc_unidad, cantidad, equivalencia, montound, descuento, descuentop, subtotalund, desc_impuesto, tasaimp, montoimp, totalund);
                    mForm.ShowDialog();
        ...
        private void BtnArtCargar_Click(object sender, EventArgs e)
        {
            ...
            else if (TxtEditar.Text == "Listar")
            {
                ...
                Cargar.TxtCod_compra.Text = TxtCod_fcompra.Text;
        ...
        internal void SetImportar(string cod_proveedor, string desc_proveedor, string rif, string cod_condicionpago, string desc_condicionpago, string diascredito, string desc_fcompra, string nacional, string doc_asociado, string cod_fcompra, string fecha_doc, string fecha_ven, string campo1, string campo2, string campo3, string campo4, string campo5, string campo6)
        {
            ...
            TxtDesc_condicionpago.Text = desc_condicionpago;
            TxtDiascredito.Text = diascredito;
            TxtDesc_fcompra.Text = desc_fcompra;
            TxtNacional.Text = nacional;
            TxtDoc_asociado.Text = doc_asociado;
            TxtOrigen.Text = cod_fcompra;
        ...
        private void BtnAceptarImp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                desc_compra = TxtDesc_fcompra.Text;
                nacional = TxtNacional.Text;
                doc_asociado = TxtOptipo.Text;
                origen = TxtCod_fcompra.Text;
                ...
                FCCompra ImpC = FCCompra.GetForm();
                FPCompra ImpP = FPCompra.GetForm();
                FFCompra ImpF = GetForm();
                ...
                else if (TipoImp == "FCompra" && Importar == true)
                {
        ...
        private void BtnProcesar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    if (Rpta.Equals("Y"))
                    {
                        MensajeOk("Registro Procesado Correctamente !");
                        Botones();
                        Listar();
                    }
                    else
                ...
                {
                    MensajeError("Cancelando el Proceso de Registro !");
                }
            }
            catch (Exception Exc)
            {
