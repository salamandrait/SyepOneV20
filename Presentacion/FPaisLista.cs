...
{
    public partial class FPaisLista : Form
    {
        ...
        private void OcultarColumnas()
        {
            if (DataListado.Rows.Count != 0)
            {
                ...
                DataListado.Columns["cod_pais"].Visible = true;
                DataListado.Columns["desc_pais"].Visible = true;
                ...
                DataListado.Columns["cod_pais"].Width = 100;
                DataListado.Columns["desc_pais"].Width = 350;
            ...
            DataListado.Columns["cod_pais"].HeaderText = "Codigo";
            DataListado.Columns["desc_pais"].HeaderText = "Descripcion";
        ...
        private void Mostrar()
        {
            try
            {
                DataListado.DataSource = EPais.Mostrar();
                OcultarColumnas();
            }
            catch (Exception Exc)
            {
        ...
        private void Buscar()
        {
            ...
            {
                DataListado.DataSource = EPais.Buscar(TxtBuscar.Text, "cod");
            }
            else if (TxtBtipo.Text == "Descripcion")
            {
                DataListado.DataSource = EPais.Buscar(TxtBuscar.Text, "Desc");
            }
        }

        private void FPaisListar_Load(object sender, EventArgs e)
        {
            Text = "...::: Listado :::...";
            Mostrar();
            CbBtipo();
        ...
        private void DataListado_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                FPais mForm = FPais.GetForm();
                string cod_pais, desc_pais, cod_moneda,desc_moneda, campo1, campo2, campo3, campo4, campo5, campo6;
                ...
                mForm.SetPais(cod_pais, desc_pais, cod_moneda,desc_moneda,campo1, campo2, campo3, campo4, campo5, campo6);
                mForm.TxtEditar.Text = "Listar";
            }
            if (TxtFlag == "Prov")
            {
                FProveedor mForm = FProveedor.GetForm();
                string cod_pais, desc_pais, cod_moneda, desc_moneda;
                cod_pais = Convert.ToString(DataListado.CurrentRow.Cells["cod_pais"].Value);
                desc_pais = Convert.ToString(DataListado.CurrentRow.Cells["desc_pais"].Value);
                cod_moneda = Convert.ToString(DataListado.CurrentRow.Cells["cod_moneda"].Value);
                desc_moneda = Convert.ToString(DataListado.CurrentRow.Cells["desc_moneda"].Value);
                mForm.SetPais(cod_pais, desc_pais, cod_moneda, desc_moneda);
            }
            Close();
        }
            ...
            try
            {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DataListado.Rows)
                    {
                        ...
                        {
                            Rpta = EPais.Eliminar(Convert.ToString(row.Cells["cod_pais"].Value));
                        }
                    }
