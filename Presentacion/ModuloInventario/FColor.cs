...
namespace Presentacion.ModuloInventario
{
    public partial class FColor : Form
    {
        ...
        private static FColor _Instancia;

        public static FColor GetForm()
        {
            if (_Instancia == null)
            {
                _Instancia = new FColor();
            }
            return _Instancia;
        }
        ...
        private void Guardar()
        {
            try
            {
                ...
                if (TxtCod_color.Text == string.Empty || TxtDesc_color.Text == string.Empty)
                {
                    MensajeError("Falta ingresar algunos datos");
                    ErrorTexto.SetError(TxtCod_color, "Campo Obligatorio");
                    ErrorTexto.SetError(TxtDesc_color, "Campo Obligatorio");
                }
                else
                {
                    if (TxtEditar.Text == "Nuevo")
                    {
                        Rpta = EColor.Insertar(TxtCod_color.Text.Trim().ToUpper(), TxtDesc_color.Text.Trim(), TxtCampo1.Text, TxtCampo2.Text, TxtCampo3.Text, TxtCampo4.Text, TxtCampo5.Text, TxtCampo6.Text);
                    }
                    else if (TxtEditar.Text == "Editar")
                    {
                        Rpta = EColor.Editar(TxtCod_color.Text.Trim().ToUpper(), TxtCod_colorv.Text, TxtDesc_color.Text.Trim(), TxtCampo1.Text, TxtCampo2.Text, TxtCampo3.Text, TxtCampo4.Text, TxtCampo5.Text, TxtCampo6.Text);
                    }
                    ...
                    if (Rpta.Equals("Y"))
                    {
                        if (TxtEditar.Text == "Nuevo")
                        {
                            ...
                            TxtEditar.Text = "Listar";
                            TxtCod_colorv.Text = TxtCod_color.Text;
                        }
                        else if (TxtEditar.Text == "Editar")
                        {
        ...
        private void GenerarCod()
        {
            try
            {
                int rpta = Convert.ToInt32(EColor.GenerarCod(TxtCod_color.Text));
                TxtCod_color.Text = Pref + rpta.ToString(TxtCadena.Text);
            }
            catch (Exception Ex)
            {
        ...
        internal void SetColor(string cod_color, string desc_color, string campo1, string campo2, string campo3, string campo4, string campo5, string campo6)
        {
            TxtCod_color.Text = cod_color;
            TxtCod_colorv.Text = cod_color;
        ...
        private void FColor_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Instancia = null;
        }

        private void FColor_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            //FRptColor Reporte = new FRptColor();
            //Reporte.ShowDialog();
        }
        ...
        private void BtnAnterior_Click(object sender, EventArgs e)
        {
            try
            {
                Ds = EColor.Anterior(TxtCod_color.Text);
        ...
        private void BtnSiguiente_Click(object sender, EventArgs e)
        {
            try
            {
                Ds = EColor.Siguiente(TxtCod_color.Text);
        ...
        private void TxtCod_color_TextChanged(object sender, EventArgs e)
        {
            CambiosTexBox();
        }
